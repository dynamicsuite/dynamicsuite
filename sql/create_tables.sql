...
CREATE TABLE IF NOT EXISTS `ds_view_group_permissions` (
    ...
    `package_id` VARCHAR(64) NULL COLLATE 'utf8mb4_general_ci',
    `name` VARCHAR(64) NULL COLLATE 'utf8mb4_general_ci',
    `domain` VARCHAR(64) NULL COLLATE 'utf8mb4_general_ci',
    `description` VARCHAR(255) NULL COLLATE 'utf8mb4_general_ci'
) ENGINE=MyISAM;
...
CREATE VIEW IF NOT EXISTS `ds_view_group_permissions` AS select `data`.`group_id` AS `group_id`,`meta`.`permission_id` AS `permission_id`,`meta`.`package_id` AS `package_id`,`meta`.`name` AS `name`,`meta`.`domain` AS `domain`,`meta`.`description` AS `description` from (`ds_group_permissions` `data` left join `ds_permissions` `meta` on(`data`.`permission_id` = `meta`.`permission_id`));

DROP TABLE IF EXISTS `ds_view_user_groups`;
CREATE VIEW IF NOT EXISTS `ds_view_user_groups` AS select `data`.`user_id` AS `user_id`,`meta`.`group_id` AS `group_id`,`meta`.`name` AS `name`,`meta`.`domain` AS `domain`,`meta`.`description` AS `description` from (`ds_user_groups` `data` left join `ds_groups` `meta` on(`data`.`group_id` = `meta`.`group_id`));

DROP TABLE IF EXISTS `ds_view_user_permissions`;
CREATE VIEW IF NOT EXISTS `ds_view_user_permissions` AS select `meta`.`user_id` AS `user_id`,`meta`.`group_id` AS `group_id`,`data`.`permission_id` AS `permission_id`,`data`.`package_id` AS `package_id`,`data`.`name` AS `name`,`data`.`domain` AS `domain`,`data`.`description` AS `description` from (`ds_user_groups` `meta` left join `ds_view_group_permissions` `data` on(`data`.`group_id` = `meta`.`group_id`)) group by `meta`.`user_id`,`data`.`permission_id`;
