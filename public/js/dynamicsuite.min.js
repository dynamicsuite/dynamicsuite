...
class DynamicSuite{static call(t,e,a){fetch(`/dynamicsuite/api/${t}`,{method:"POST",body:JSON.stringify(e||[])}).then(t=>{if(t.ok)return t;throw new Error("A server error has occurred")}).then(t=>t.json()).then(t=>a(t),()=>a({status:"SERVER_ERROR",message:"A malformed response was returned",data:null})).catch(t=>{console.log(t),a({status:"SERVER_ERROR",message:"A malformed response was returned",data:null})})}static readCustomData(t){return!(!window.dynamicsuite.hasOwnProperty("custom")||!window.dynamicsuite.custom.hasOwnProperty(t))&&window.dynamicsuite.custom[t]}static broadcast(t,e){void 0!==this.vm&&this.vm.$emit(t,e)}static listen(t,e){void 0!==this.vm&&this.vm.$on(t,e)}static setURLSavedData(t,e,a=!0){const n=new URLSearchParams(window.location.search);n.set(t,e);e=n.toString(),e=`${window.location.pathname}?${e}`;a?history.pushState({},null,e):history.replaceState({},null,e)}static deleteURLSavedData(t,e=!0){const a=new URLSearchParams(window.location.search);a.delete(t);t=a.toString(),t=t?`${window.location.pathname}?${t}`:window.location.pathname;e?history.pushState({},null,t):history.replaceState({},null,t)}static clearURLSavedData(t){const e=new URLSearchParams(window.location.search);for(const a of t)e.delete(a);t=e.toString(),t=t?`${window.location.pathname}?${t}`:window.location.pathname;history.replaceState({},null,t)}}window.addEventListener("load",()=>{DynamicSuite.vm=new Vue({name:"DynamicSuite",el:"#dynamicsuite",data(){return{selected_group:null,has_session:!1,hide_overlay:!0,default_view:null,overlay_nav_tree:[],overlay_nav_footer_text:null,overlay_nav_footer_view:null,overlay_nav_alert_success:{},overlay_nav_alert_warning:{},overlay_nav_alert_failure:{},overlay_title:null,overlay_actions:[]}},methods:{setNavAlert(t,e,a){this.$set(this[`overlay_nav_alert_${t}`],e,a)},setNavActive(t){this.$refs.ds_overlay.setNavActive(t)}},mounted(){for(const t of Object.keys(this._data))window.dynamicsuite.hasOwnProperty(t)&&(this[t]=window.dynamicsuite[t]);document.getElementById("dynamicsuite").style.display="flex",location.hash&&(location.href=location.hash)}})});